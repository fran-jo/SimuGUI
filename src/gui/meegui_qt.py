...
class Ui_Dialog(object):
    ...
    lfile=""
    opath=""
    fname1='simParametersOMC3.properties'
    ...
    def Select_Model(self):
        ...
        if self.fname:
            ...
            self.directori='/'.join(self.MF[:-1])
            self.filename=''.join(self.MF[-1:])
            self.mname=self.filename
            self.source1.set_modelName(str(self.filename))
            self.lineEdit_3.setText(self.fname)
        else:
            self.lineEdit_3.setText('No file selected')
    ...
    def get_fname_save_Resource(self): 
        ...
        self.source1.set_modelName(self.mname)
